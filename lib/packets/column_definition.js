...
// reading few bytes from buffer). name, schema, and table are
// used frequently (in wrap function for every field), so we
// precompute them. Other properties are unlikely to be used,
// so we postpone their string conversion until property access
//
...
const addString = function (name, cacheName) {
...
// Use separate cache properties to avoid conflicts and for faster access
addString('catalog', '_cachedCatalog');
addString('schema', '_cachedSchema');
addString('table', '_cachedTable');
addString('orgTable', '_cachedOrgTable');
addString('orgName', '_cachedOrgName');
